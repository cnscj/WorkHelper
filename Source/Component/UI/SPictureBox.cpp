...
SPictureBox::SPictureBox(QWidget *parent) : QWidget(parent)
{
...
void SPictureBox::setBackground(QBrush brush)
{
    m_brush = brush;
    update();
}

void SPictureBox::setMode(PB_MODE mode)
{
    m_mode = mode;
    if(m_mode == AUTO_SIZE)
    {
...
bool SPictureBox::setImage(QImage &image, double scale)
{
    if(image.isNull())
    {
        return false;
...
void SPictureBox::paintEvent(QPaintEvent * event)
{
    Q_UNUSED(event);
